...
#Configure Cart
resource "terraform_data" "cart_setup" {
  triggers_replace = [module.cart_server.instance_id[0]]
  depends_on = [ time_sleep.wait_for_cart ]
  connection {
    host = module.cart_server.private_ip[0]
  ...
  provisioner "remote-exec" {
    inline = [ "chmod +x /tmp/bootstrap.sh",
               "sh /tmp/bootstrap.sh cart ${var.environment} ${var.project}" ]
  }
}
